...
: MTmp123 ( -- )  \ setup Tmp1 & Tmp2 & Tmp3
  [ MSPtr lda  MSPtr 1+ ldy              Tmp1 sta.z  Tmp1 1+ sty.z  \ Tmp1= ^ top of MStack
...
: MSAlloc ( -- m )  \ alloc a new M stack cell
...
: M0=P ( M: m -- m P.z )  \ Check if top data stack cell is empty.
  MTmp123 [
...
: M0<P ( M: m -- m P.m )  \ Is top data cell negative?
  MTmp123 [  MCellBytes ldy  dey  Tmp1 lda.ziy ] ;  SeeL

: MCmp ( M: m1 m2 -- m1 m2 ) ( -- P.ncz )  \ compare, set CPU flags NCZ
  MTmp123 [  \ setup Tmp1 & Tmp2
  LSInit  \ init local labels
  MCellBytes ldy  dey
  Tmp1 lda.ziy  sec  Tmp2 sbc.ziy  LRefR @18 beqa  \ compare hi word
    LRefR @NFlip bvsa  rts

 LDef @11  Tmp1 lda.ziy  Tmp2 cmp.ziy  LRefR @Ne bnea
...
: MPad ( A Y -- ) ( M: m1 -- m2 ) \ fill hi bytes of m
  [ LSInit
...
: MU>D ( M: um1 -- um1 0 )  \ convert unsigned um1 to double unsigned m
  0 U>M ;  SeeL
: MS>D ( M: m1 -- m1 m )  \ convert signed m1 to double m
...
: MRol ( M: m1 P.C -- m2 P.C )  \ Rotate the bits of m1 left through the carry bit.
  [ php ] MTmp123 [ plp  \ setup Tmp1
  Tmp2 stx.z  \ save data stack index
  MCellBytes ldx  0 ldy.#  Begin,
    Tmp1 lda.ziy  rol.a  Tmp1 sta.ziy  iny   dex  UntilEq,
...
: M+1 ( M: m1 m2 -- m3 )  \ Add m1 to m2 giving m3
...
: M-1 ( M: m1 m2 -- m3 )  \ Subtract m2 from m1 giving m3
  \ Strange name so we don't redefine M+ from standard FORTH.
...
: MDNegate ( M: mlo1 mhi1 -- mlo2 mhi2 )  \ return two's complement of double cell.
...
: Hex>M ( uhi ... ulo -- ) ( M: -- m ) \ convert words to M
  MSAlloc [  \ Tmp1= ^new MCell
  0 ldy.#  Begin,
    DStack lda.zx  inx  Tmp1 sta.ziy  iny  MCellBytes cpy UntilCs,
  ] ;  SeeL
: M.Hex ( M: m -- )  \ type m in hex
  [ MCellBytes ldy  dey  Begin, tya  pha
    ] MTmp123 [ pla  pha  tay  Tmp1 lda.ziy ] PushZA C.Hex [
    pla  tay  dey  UntilMi,
  ] MDrop ;  SeeL
    
: MU*ByteA ( M: m1 -- m2 ) ( A -- ) \ return unsigned m*n
  [ Tmp5 sta.z ]  \ save multiplier
  ...
  Begin, Tmp5 lsr.z  \ for each multiplier bit
    ...
    clc  0 ldy.#  Begin,  \ double work
      Tmp1 lda.ziy  rol.a  Tmp1 sta.ziy   iny  tya  MCellBytes eor  UntilEq,
    Repeat,
  ' MDrop jmp ] ;  SeeL
...
: MUM* ( M: um1 um2 -- umlo umhi ) \ Multiply unsigned giving double
  ...
  MSAlloc  \ alloc work mcell
  MTmp123 [  \ Tmp1=^work, Tmp2=^umhi, Tmp3=^umlo
  Tmp5 stx.z  \ save data stack index
...
: MU* ( M: um1 um2 -- um3 )
  MUM* MDrop ;  SeeL
: M*1 ( M: m1 m2 -- m3 )  \ multiply m1 by m2 giving m3
...
: MU/ModByteA ( M: um1 -- umquotient ) ( udivisor -- A=uremainder )
  PopA [ Tmp5 sta.z  \ save divisor
  ] M2* [  \ get 1st dividend bit
  php  MCellBytes lda  asl.a  asl.a  asl.a  Tmp5 1+ sta.z  plp  \ # of bits
  0 lda.#  \ init top byte of dividend
  Begin,  \ for each dividend bit
    rol.a
    Tmp5 cmp.z  IfCs, Tmp5 sbc.z Then,
    pha  ] MRol [ pla
   Tmp5 1+ dec.z  UntilEq,
  ] ;  SeeL
...
: MUM/Mod ( M: umlo umhi umdivisor -- umremainder umquotient )  \ division
  ...
  MCellBytes ldy  Begin, dey  Tmp5 1+ sty.z  \ for each umlo/umquotient byte
    ...
    8 lda.#  Tmp5 sta.z  Begin,  \ for each bit in byte
      MCellBytes ldx  0 ldy.#  Begin, Tmp2 lda.ziy rol.a Tmp2 sta.ziy iny  dex UntilEq,  \ rol umhi
      LRefR @sub bcsa
...
: MU/Mod ( um-dividend um-divisor -- um-remainder um-quotient ) \ Divide unsigned
  MSAlloc MTmp123 [
...
: MSM/Rem ( mlo mhi mdivisor -- mremainder mquotient ) \ Symmetric signed division
