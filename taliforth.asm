...
; Dictionary headers are together with the code, which allows the "start of code"
; pointer to be omitted and code length to be 1 byte.
...
CO =   1 ; Compile Only
AN =   2 ; Always Native Compile (may not be called by JSR)
IM =   4 ; Immediate Word
NN =   8 ; Never Native Compile (must always be called by JSR)
UF = $10 ; Includes strippable Underflow Check
;    $20 ; unused
R6 = $40 ; strip 6 bytes from begin & end when native compiling
;    $80 ; unused
wh_xt:			;  start of 6502 code
 .endvirtual
...
;  The fields above are at negative offsets from xt.
WordListLink ::= *-wh_xt ; remember the nt of this word for later
			;wh_xt	6502 code starts here.  This is the address that "find" returns.
...
FEnd .macro  ; fill in word header's code length
CL	.var *-(WordListLink+wh_Xt)
	.if CL>$ff
;	  .assert DictFlags & NN, warning, "code size >255 & not marked NN"
CL	 .var $ff
	 .endif
en = *	; remember here
...
UnderflowCheck .macro depth,ErrorDestination ; compile a stack underflow check
...
WordListLink .var 0	 ; start of FORTH wordlist
 ...
 FHdr "Cold",NN ; ( -- ) "Reset the Forth system"
...
Cold:
		...
		; Initialize important zero page variables from ROM
		ldx #Cold_zp_table_length-1
-		; We can use X here
		; because Tali hasn't set up the param stack yet.
		lda Cold_zp_table,x
		sta zp0,x
		dex
		bpl -
...
-		lda Cold_user_table,y
  ...
  .if user_words_len>0
		; define user words via EVALUATE.
...
Bye:		jmp platform_bye
	FEnd


 FHdr "TypeSymbol",NN ; ( addr -- )  print addr as symbol
TypeSymbol:	jsr Space
		jsr Dup
		lda #'$'
		jsr Emit_A
		jsr Dot_Hex

TypeSymbolN: ; without hex version
		jsr Space
		lda #'('
		jsr Emit_A
		jsr Space
...
; FHdr "DictSearchXt",NN ; ( xt -- offset nt ) search wordlists for word with best match
DictSearchXt:
		dex
		dex
		jsr Over
...
_WordNext:	ldy #wh_WordListLink+1
		lda (tmp1),y
...
_WordTest:	clc			; tmp2 = xt of word
		...
		sec			; tmp2 = xt offset for this word
		lda DStack+0,x
		sbc tmp2+0
		sta tmp2+0
		lda DStack+1,x
		...
		lda tmp2+0		; this word have smaller offset?
		cmp DStack+4,x
		lda tmp2+1
		sbc DStack+5,x
		bcs _WordNext

		lda tmp2+0		; save this better offset
		ldy tmp2+1
		sta DStack+4,x
		sty DStack+5,x
...
; Random #s ------------------------------------------------------
 ...
 FHdr "RndState",NN ; ( -- addr )  Double variable: random # state
		lda #<RndState
		ldy #>RndState
		jmp PushYA
	FEnd
...
; FHdr "Rand",NN ; ( -- )  generate next random #
...
;	FEnd
 ...
 FHdr "RandM",NN ; ( umod - u )  random integer between 0 & umod-1
RandM:		jsr Dup		; ( umod umod )
		jsr Rand
		lda RndState+2
		ldy RndState+3
		sta DStack+2,x
		sty DStack+3,x	; ( rand umod )
		jmp Mod
	FEnd
...
FSMant3: .fill FDim	; FP stack mantissa 4th array
 ...
 FHdr "FDim",NN ; ( -- n )  Constant: # of FP stack entries
		lda #FDim
		jmp PushZA
	FEnd
 ...
 FHdr "FMant1",NN ; ( -- addr )  Byte variable: FP mantissa 2nd array
		lda #FSMant1
		jmp PushZA
	FEnd

 FHdr "FMant2",NN ; ( -- addr )  Byte variable: FP mantissa 3rd array
		lda #FSMant2
		jmp PushZA
	FEnd

 FHdr "FMant3",NN ; ( -- addr )  Byte variable: FP mantissa 4th array
		lda #FSMant3
		jmp PushZA
	FEnd


; FHdr "Float",NN ; ( -- u )  size of a float  gforth
;		lda #5
;		jmp PushZA
;	FEnd

 FHdr "Float+",NN ; ( r-addr1 -- r-addr2 )  Add the size in address units of a floating-point number.
...
FDepth:		lda #FDim
 ...
 FHdr "FAllocX",NN ; ( F: -- r )  alloc a FP stack entry, returns X=fp
FAllocX:	stx tmp1+0	; save data stack index
		ldx fp		; X= floating point stack index
		cpx #FDim+1	; underflow?
		bcs _underflow
...
_overflow:	ldx tmp1+0	; restore data stack index
		lda #$100+err_FPStackOverflow
		jmp ThrowA

_underflow:	ldx tmp1+0	; restore data stack index
		lda #$100+err_FPStackUnderflow
		jmp ThrowA
	FEnd


 FHdr "FMax",NN ; ( F: r1 r2 -- r3 )  r3 is the greater of r1 and r2. 
...
FRot:		stx tmp1		; save data stack index
	...
	.cerror FSMant3-FSMant2!=FDim
		...
		lda fp			; for FSMant3, FSMant2, FSMant1, FSMant0, FSExp
		clc
		adc #4*FDim
		bne _3
...
;---
 ...
 FHdr "F-Rot",NN ; ( F: r1 r2 r3 -- r3 r1 r2 )
FMRot:		jsr FRot
		jmp FRot
	FEnd
...
FZLe:		ldy fp
 ...
 FHdr "F<",NN ; ( F: r1 r2 -- ) ( -- flag )  return r1 < r2
...
FEq:		jsr FCmpA
 ...
 FHdr "F~Abs",NN ; ( F: r1 r2 r3 -- flag )  approximate equality with absolute error |r1-r2|<r3
FTAbs:		jsr FMRot
		jsr FMinus
		jsr FAbs
		jmp FGt
...
_10:		jsr FNegate
 ...
 FHdr "FVariable",IM+NN ; ( "name" -- )  Create FP variable (internal format)
...
FVariable:	jsr Create		; compile word header & push PFA adr
 ...
 FHdr "FLiteral",IM+NN ; ( F: r -- )  Compile FP literal
...
FLiteral:	ldy fp
		...
		beq _Short
					...
					; full precisision literal
		lda #<FLitI		; compile FLitI call
		ldy #>FLitI
		jsr Jsr_Comma_YA
...
_Short:
		dex
		dex
		lda FSExp,y		;compile load value
		sta DStack+1,x
		lda FSMant0,y
		sta DStack+0,x
		jsr ldya_immed_comma
		inc fp			; FDrop
		lda #<FLitYA		; compile jsr FLitYA, & return
		ldy #>FLitYA
		jmp Jsr_Comma_YA
	FEnd
...
F1Plus:		jsr F1
 ...
 FHdr "F-",NN ; ( F: r1 r2 -- r3 )  Subtract r2 from r1, giving r3.
...
FNegate:	stx tmp1	; save data stack index
 ...
 FHdr "FAbs",NN ; ( F: r1 -- r2 )  Absolute value of r.
...
; FHdr "FPos",NN ; ( F: r1 r2 -- r1 r2 )  make r1 & r2 positive & return result sign
FPos:		ldy fp			; load FP stack index
...
F10Star:	jsr FDup
 ...
 FHdr "F/",NN ; ( F: r1 r2 -- r3 )  Divide r1 by r2, giving r3.
...
_2:		rol tmp1+1		;   accum quotient bits
 ...
 FHdr "F1/",NN ; ( F: r1 -- r2 )  FP reciprocal
F1Slash:	jsr F1
		jsr FSwap
		jmp FSlash
	FEnd
...
ESlash:		jsr FPos	; make r1 & r2 positive
		...
		jsr _Byte		; get quotient hi byte
		pha
		jsr _Byte		; get quotient lo byte
		sta FSMant1+1,x
		pla
		lsr a
		sta FSMant0+1,x
...
E1Slash:	jsr F1
 ...
 FHdr "S>F",NN ; ( n -- ) ( F: -- r )  convert n to r.  aka N>F
...
SToFYA:		jsr FAllocX		; alloc FP stack entry
 ...
 FHdr "D>F",NN ; ( d -- ) ( F: -- r )  convert d to r
...
FDToF:		jsr FAllocX		; alloc FP stack entry
 ...
 FHdr "F>S",NN ; ( F: r -- ) ( -- n )  Convert r to n.  aka F>N
...
_overflow:	plp			; RDrop saved sign
 ...
 FHdr "F>D",NN ; ( F: r -- ) ( -- d )  convert r to d
...
_overflow:	plp			; RDrop sign
 ...
 FHdr "FRnd",NN ; ( F:  -- r )  get random # between 0 & 1
FRnd:		jsr Rand		; generate next RndState
		jsr FAllocX		; alloc FP stack entry
		lda #0			; exponent=0
		sta FSExp,x
		lda RndState+1		; mantissa = RndState
		lsr a			;   make positive
		sta FSMant0,x
		lda RndState+0
		sta FSMant1,x
...
_19:		rts
 ...
 FHdr "(F.)",NN ; ( F: r -- ) ( -- adr )  Convert FP to string, fixed-point
  ; Display, with a trailing space, using fixed-point notation: 
  ;	[-] <digits>.<digits0>
PFDot:		jsr One		;alloc & init work area
		lda PrecisionV
		jsr PushZA
_decimalPos   = DStack+2	; decimal point position
_mantNzFound  = DStack+1	; nonzero mantissa digit processed
_NumSigDigits = DStack+0	; # of significant digits

		jsr pfcst		;start collecting chars

		jsr FMSign		;do mantissa sign

_a:		ldy fp
		lda FSExp,y		;scale down to <10
...
_d9:
 ...
 FHdr "F.",NN ; ( F: r -- )  Print r in fixed-point notation
...
FDot2:		jsr Count
 ...
 FHdr "F.S",NN ; ( -- )  Non-destructive print of the float stack contents.
...
; FHdr "FE.",NN ; ( F: r -- )  Print r in engineering format
 ...
 FHdr "(FS.)",NN ; ( F: r -- adr )  Convert r to scientific notation string
...
_3:		ldy fp
		lda FSMant0,y
		beq _39
		lda FSExp,y		;while fp < 1
		beq _31
		bpl _39
_31:		dec DStack+0,x		;  fp*=10
		jsr F10Star
		jmp _3
_39:

_4:		jsr pedotd		;do mantissa digits
		lda #'.'
		jsr pfch
		jsr pedotd
		jsr pedotd
...
pedotd: ; do a mantissa digit
 ...
 FHdr "FS.",NN ; ( F: r -- )  Print r in scientific notation.
...
FSDot:		jsr PFSDot
 ...
 FHdr 'F"',IM+NN ; ( "string" -- ) ( F: -- r )  get inline string as float
...
_Err:		lda #100+err_FPInvalidArg
 ...
 FHdr ">Float",NN ; ( F: -- r ) ( c-addr u -- true | false)  Convert string to FP
...
; $3e200000 = 0.15625,  $c2ed4000 = -118.625
 ...
 FHdr "SF@",NN ; ( sf-addr -- ) ( F: -- r )  Fetch IEEE single
...
+
 ...
 FHdr "SF!",NN ; ( F: r -- ) ( sf-addr -- )  Store FP as IEEE single
...
+
		...
		stx tmp1+0		; save data stack index
		ldx fp			; switch to FP stack index
...
; FHdr "SFAlign",NN ; ( -- )  Align compile ptr for IEEE single
...
; FHdr "SFAligned",NN ; ( addr -- sf-addr )  Align addr for IEEE single
...
;_9:		rts
 ...
 FHdr "SFloat+",NN ; ( sf-addr1 -- sf-addr2 )  Add size of IEEE single to addr
...
; $3e200000 = 0.15625,  $c2ed4000 = -118.625
 ...
 FHdr "DF!",NN ; ( F: r -- ) ( df-addr -- )  Store FP in IEEE double format
...
_shift:		lsr FSMant0,x
 ...
 FHdr "DF@",NN ; ( df-addr -- ) ( F: -- r )  Fetch IEEE double
...
; FHdr "DFAlign",NN ; ( -- )  Align compile ptr for IEEE double
...
; FHdr "DFAligned",0 ; ( addr -- df-addr )  Align addr for IEEE double
...
;_9:		rts
 ...
 FHdr "DFloat+",NN ; ( df-addr1 -- df-addr2 )
...
_4:		lda #5			; bump coefficent ptr
 ...
 FHdr "FLog2M1M",NN ; ( r -- r )  Minimax polynomial approximations for log2(x)
...
_OutOfRange:	jmp Throw_FpOutOfRange
 ...
 FHdr "FLn",NN ; ( F: r1 -- r2 )  (base e) natural log
...
FLn:		jsr FLog2
 ...
 FHdr "FLnP1",NN ; ( F: r1 -- r2 ) (base e) natural log
  ; https://forth-standard.org/standard/float/FLNPOne
FlnP1:		jsr F1Plus
		jmp FLn
	FEnd

 FHdr "FLog",NN ; ( F: r1 -- r2 )  (base 10) common log
...
FLog:		jsr FLog2
 ...
 FHdr "FExp2M1M",NN ; ( F: r1 -- r2 )  2^r1-1 for r1 in [0,1]
...
FExp2:		jsr FIntFrc	; ( f: r_rem r_int )
 ...
 FHdr "FExp",NN ; ( F: r1 -- r2 )  base e (natural) exponential
...
FExp:		FloatLitI $5c551d94,1	;1/ln(2)
 ...
 FHdr "FExpM1",NN ; ( F: r1 -- r2 ) base e (natural) exponential
...
FALog:		FloatLitI $6a4d3c20,2	;1/log(2)
 ...
 FHdr "F**",NN ; ( F: r1 r2 -- r3 )  Raise r1 to the power r2 
...
FPower:		jsr FSwap
 ...
 FHdr "Deg2Rad",NN ; ( F: r1 -- r2 )  Convert degrees to radians.
Deg2Rad:	FloatLitI $477d1A8A,-5	; PI/180
		jmp FStar
	FEnd

 FHdr "Rad2Deg",NN ; ( F: r1 -- r2 )  Convert radians to degrees.
Rad2Deg:	FloatLitI $72977068,6	; 180/PI
		jmp FStar
	FEnd


 FHdr "FAReduce",NN ; ( F: r1 -- r2 )  C=mirrored around pi/2
...
+
 ...
 FHdr "FACos",NN ; ( F: r1 -- r2 )  Inverse cosine
...
.if "ed" in TALI_OPTIONAL_WORDS
 ...
 FHdr "Ed",NN ; ( -- u ) "Line-based editor"
...
+
		...
		lda #2
		jsr plus_a		; ( addr-t u-t here here2+2 )
...
.endif ; ed
 ...
 FHdr "See",NN ; ( "name" -- ) "Print information about a Forth word"
...
_flag_loop:	jsr Space
		...
		jsr Swap		; ( xt nt )
		jsr WordSize		; ( xt u )
		jsr Dup			; ( xt u u ) for DUMP and DISASM
		jsr Decimal
		jsr U_Dot		; ( xt u )
		jsr CR
...
Forth_WordList:	jmp Zero
 ...
 FHdr "Editor-WordList",NN ; ( -- u ) "WID for the Editor wordlist"
...
Editor_WordList:
 ...
 FHdr "Assembler-WordList",NN ; ( -- u ) "WID for the Assembler wordlist"
...
Assembler_WordList:
 ...
 FHdr "Root-Wordlist",NN ; ( -- u ) "WID for the Root (minimal) wordlist"
...
_string:	asl		; Get the string based on WID 0 to 3
		tay
		lda _wid_Table+0,y
		pha
		lda _wid_Table+1,y
		...
		jmp Print_ASCIIZ_YA_no_lf ; Print without a line feed
	FEnd

_wid_Table: ; string names (see strings.asm) indexed by the WID if < 4 .
	.word str_wid_forth	       ; WID 0: "Forth"
	  .cerror wid_Forth!=0
	.word str_wid_editor	       ; WID 1: "Editor"
	  .cerror wid_Editor!=1
	.word str_wid_asm	       ; WID 2: "Assembler"
	  .cerror wid_Assembler!=2
	.word str_wid_root	       ; WID 3: "Root"
	  .cerror wid_Root!=3
...
WordList:
		...
		lda Num_wordlistsV	; This is a byte variable
...
+
	...
	;	clc			; Increment the wordlist#
		adc #1
		sta Num_WordlistsV	; Save it into byte variable #wordlists
...
_start:		; Continue processing with ( forth-wordlist 1 -- )
		...
		lda DStack+0,x
		sta Num_orderV,y	; #ORDER is a byte variable.
...
_addr: ; ( n -- addr )  point at ramdrive block
 ...
 FHdr "RamDrive",NN ; ( -- addr )  push address of RamDrive buffer
...
BuffBlockNum:	lda #<BuffBlockNumV
		ldy #>BuffBlockNumV
		jmp PushYA
	FEnd
...
Block_Read:	jmp (BlockReadV)	; Execute the BLOCK-READ-VECTOR
 ...
 FHdr "Block-Read-Vector",NN ; ( -- addr )  Address of the block-read vector
...
Update:
 ...
 FHdr "Buffer",0 ; ( u -- a-addr ) "Get a buffer for a block"
...
Thru:
		...
		; We'll use the same trick that DO loops use, holding
...
_loop:
 ...
 FHdr "List",UF+NN ; ( scr# -- ) "List the given screen"
...
Defer:
		...
		lda #<_undefined	; compile "jmp _undefined"
		ldy #>_undefined
		jsr Jmp_Comma_YA
...
_undefined:  ; Error routine for undefined DEFER:
...
Defer_Fetch:
		; No underflow checking as 1+ does it.
		jsr One_plus
		jmp Fetch
	FEnd
...
Defer_Store:
		; No underflow checking as 1+ and ! do it.
		jsr One_plus
		jmp Store
	FEnd
...
_compiling:
		jsr Bracket_Tick	; compile the xt of the next word as a literal.

		ldy #>Defer_Store	; Postpone DEFER! by compiling a JSR to it.
		lda #<Defer_Store
		jmp Jsr_Comma_YA
...
_compiling:
		...
		ldy #>Defer_Fetch	; Postpone DEFER@ by compiling a JSR to it.
		lda #<Defer_Fetch
		jmp Jsr_Comma_YA
...
Of:
		ldy #>OfX		; Check if value is equal to this case.
		lda #<OfX
		jsr Jsr_Comma_YA
...
If:
		jsr zbranch_jsr_comma	; Compile a 0BRANCH
...
Begin:		jmp Here	; remember the loop starting location
	FEnd
...
While:
		jsr If		; Compile a 0branch & jmp, push addr of addr
		jmp Swap	; Swap the two addresses on the stack.
	FEnd
...
Repeat:
		jsr Again	; compile the jmp back
...
Branch_Comma: ; compile branch.  opcode in A, addr in TOS
...
+		sta DStack+7,x
		...
		lda DStack+1,x
		sta tmp1+1
		ldy #wh_WordListLink	; get next word nt
		lda (tmp1),y
		sta DStack+0,x
		iny
		lda (tmp1),y
...
+		jmp Type
 ...
 FHdr "At-XY",UF+NN ; ( n m -- ) "Move cursor to position given"
...
At_XY:
 ...
 FHdr "<#",NN ; ( -- ) "Start number conversion"
  ; ## "<#"  auto	 ANS core
...
Less_Number_Sign:
 ...
 FHdr "#>",UF+NN ; ( d -- addr u ) "Finish pictured number conversion"
  ; ## "#>"  auto	 ANS core
...
Number_Sign_Greater:
 ...
 FHdr "Hold",NN ; ( char -- ) "Insert character at current output"
  ; ## "hold"  auto  ANS core
...
+		adc #'0'
 ...
 FHdr "#S",UF+NN ; ( d -- d )  Convert all digits in pictured output
  ; ## "#s"  auto	 ANS core
...
Throw_Underflow_14: jmp Throw_Underflow
...
UM_Star:
		UnderflowCheck 2,Throw_Underflow_14
...
M_Star:
		UnderflowCheck 2,Throw_Underflow_15
...
Star:
		; UM_Star will check for underflow
		jsr UM_Star
...
UM_Slash_Mod:
		UnderflowCheck 3,Throw_Underflow_15
...
Throw_Underflow_15: jmp Throw_Underflow


; FHdr "UD/Mod",NN ; ( ud u_div -- u_rem ud_quot )
...
SM_Slash_Rem:
		UnderflowCheck 3,Throw_Underflow_15 ; contains double number
...
Star_Slash:
 ...
 FHdr "Evaluate",UF+NN ; ( addr u -- ) "Execute a string"
...
evaluate_got_work:
		...
		lda tmp1	; See if we should zero BLK.
		bne _nozero

		; Set BLK to zero.
		; lda #0	; A is already zero
		sta BlkV+0
		sta BlkV+1
...
+
 ...
 .if 0 ; return stack based DO...LOOP
...
Question_Do:
		lda #<_runtime	; runtime routine ptr
		ldy #>_runtime
		bne do_common	; jump to common code
	FEnd


_runtime: ; runtime routine for ?DO

		lda DStack+0,x	; Are TOS and NOS equal?
		cmp DStack+2,x
		bne do_runtime
		lda DStack+1,x
		cmp DStack+3,x
...
; ## "do"  auto	 ANS core
	...
	; """
Do:		lda #<do_runtime	; runtime routine ptr
		ldy #>do_runtime
do_common: ; YA contains runtime routine ptr
		jsr PushYA		; save the ptr.

		jsr Here		; remember addr of part1
		jsr Swap
		ldy #0			; compile part1
-		lda _part1,y
		jsr C_Comma_A
		...
		jsr Jsr_Comma		; compile runtime routine call

		jsr Here		; remember loop body start addr

		lda #$48		; PHA	push index
		ldy #$98		; TYA
		jsr Comma_YA
		lda #$48		; PHA
		jmp C_Comma_A
...
_part1:
		lda #0			; push leave addr MSB (patched by LOOP/+LOOP)
		pha
		lda #0			;   "    "     "  LSB  "
		pha
		pha			; alloc return stack space for FUFA
		pha
_part1_len = *-_part1


do_runtime: ; Runtime routine for DO
  ...
  ; This routine is called (DO) in some Forths.
		...
		stx tmp1	; Y= S  to access the return stack
		tsx
		txa
		tay
		ldx tmp1

		; fudge factor (FUFA)= $8000 - limit
		;   the number that will trip the overflow flag
		sec
		lda #0
		sbc DStack+2,x	; LSB of limit
		sta RStack+3,y	; save FUFA
		lda #$80
		sbc DStack+3,x	; MSB of limit
		sta RStack+4,y	; save FUFA

		; index= FUFA plus original index
		clc
		lda DStack+0,x	; LSB of original index
		adc RStack+3,y	; add LSB of FUFA
		sta tmp1	; save index
		lda DStack+1,x	; MSB of orginal index
		adc RStack+4,y	; add MSB of FUFA
		ldy tmp1
...
-		lda plus_loop_runtime,y
		jsr C_Comma_A
		iny
		cpy #plus_loop_runtime_length
		bcc -

Plus_Loop_3:	; The address we need to loop back to is TOS. Store it so
...
-		lda #$68		; opcode for PLA
		...
		; Complete compile of DO/?DO by patching in the leave addr.
		; The address where they are located is on the Data Stack
		jsr PopYA		; pop addr of code to patch
		sta tmp1+0
		sty tmp1+1

		; Because of the way that RTS works, we don't have to save
		; CP, but CP-1
		lda cp+0		; LSB
		sec
		sbc #1
		...
		; ( R: leave_addr FUFA fudged_index my_rts )
	FEnd
		rts


plus_loop_runtime: ; Runtime compile for loop control.
	; Note we use a fudge factor for
	; loop control so we can test with the Overflow Flag.
	...
	; In some Forths, this is a separate word called (+LOOP) or (LOOP)
		...
		clc
		pla		; LSB of index
		adc DStack+0,x	; LSB of step
		tay		; temporary storage of LSB

		pla		; MSB of index
		adc DStack+1,x	; MSB of step
...
plus_loop_runtime_length = *-plus_loop_runtime


 FHdr "Unloop",AN+CO ; ( -- )(R: n1 n2 n3 ---) "Drop loop control from Return stack"
...
Unloop:
 ...
 FHdr "Leave",AN+CO ; ( -- ) "Leave DO/LOOP construct"
  ; ## "leave"  auto  ANS core
...
Leave:
		...
		rts		; jump to leave address
				; this must be compiled, so keep before FEnd
	FEnd


 FHdr "I",NN+CO ; ( -- n )(R: n -- n)  "Push DO loop index"
; ## "i"  auto	ANS core
	...
	; """
I:
		stx tmp4	; Save data stack index
		tsx		; Setup to access the return stack directly
		...
		ldx tmp4	; restore data stack index
		jmp PushAY	; push n, return
	FEnd


 FHdr "J",NN+CO ; ( -- n ) (R: n -- n ) "Push second DO loop index"
; ## "j"  auto	ANS core
	; """https://forth-standard.org/standard/core/J
	; Note we have the 1st leave_addr on the return stack & use
...
J:
 ...
 .endif ; return stack based DO...LOOP

 .if 0 ; zero-paged based DO...LOOP
...
; ## "do"  auto	 ANS core
	...
	; """
Do:		lda #<do_run1		; runtime routine ptr
		ldy #>do_run1
do_common: ; YA contains runtime routine ptr
		jsr PushYA		; save the runtime ptr.

		jsr Here		; remember addr of part1 for later patching
...
-		lda _part1,y
		...
		jsr Here		; compile the return addr
		lda #7
		jsr Plus_A
		jsr ldya_immed_comma	;   compile "ldy #; lda #" using TOS

		jsr Swap
		jsr Jmp_Comma		; compile runtime routine call

		jmp Here		; remember loop body start addr
	FEnd
...
do_run1: ; Runtime subroutine for DO
  ; Note that ANS loops quit when the
  ; boundry of limit-1 and limit is reached, a different mechanism than
  ; the FIG Forth loop (you can see which version you have by running
  ; a loop with start and limit as the same value, for instance
...
do_run_3:
		lda DoFuFa+1		; save previous DoFuFa
		pha
		lda DoFuFa+0
		pha
		lda DoIndex+1		; save previous DoIndex
		pha
		lda DoIndex+0
		pha
...
_loop_runtime:
	...
	; """
		...
		adc #0
		sta DoIndex+1
_r3:		; If V flag is set, we're done looping and continue
		; after the +LOOP instruction

_loop_runtime_length = *-_loop_runtime
...
Plus_Loop_3:	; The address we need to loop back to is TOS. Store it so
		...
		ldy #0			; compile 2nd part
-		lda plus_loop_run_2,y
		jsr C_Comma_A
		iny
		cpy #plus_loop_run_2_length
		bcc -
...
plus_loop_run: ; Runtime compile for loop control.
	; Note we use a fudge factor for
	; loop control so we can test with the Overflow Flag.
	; See the Control Flow section of the manual for details.
	; The step value is TOS in the loop.
	; In some Forths, this is a separate word called (+LOOP) or (LOOP)
		...
		; If V flag is set, we're done looping

plus_loop_run_length = *-plus_loop_run


plus_loop_run_2: ; Runtime, 2nd part
		pla		; restore previous DoIndex
		sta DoIndex+0
		pla
		sta DoIndex+1
		pla		; restore previous DoFuFa
		sta DoFuFa+0
		pla
		sta DoFuFa+1
		rts		; using leave addr
plus_loop_run_2_length = *-plus_loop_run_2
...
Plus_Loop_5:	jsr Loop_End		; compile JSR _runtime, BVC back
		...
		lda #<Unloop
		ldy #>Unloop
		jmp Jsr_Comma_YA	; compile JSR Unloop, return
	FEnd

Plus_Loop_Runtime:
		UnderflowCheck 1,Throw_Underflow_12
...
Throw_Underflow_12: jmp Throw_Underflow
...
_p9:
 ...
 FHdr "Unloop",NN+CO ; ( -- )(R: n1 n2 n3 ---) "Drop loop control from Return stack"
  ; ## "unloop"  auto  ANS core
  ; https://forth-standard.org/standard/core/UNLOOP
Unloop:
		ldy DoStkIndex
...
Abort:		lda #$100+err_Abort
 ...
 FHdr "?Stack",NN ; ( -- )  Check stack bounds
QStack:		txa
		bmi Throw_Overflow
		cpx #DStack0+1		; check data stack (UnderflowCheck)
		bcs Throw_Underflow

		; could check return stack

 .if "fp" in TALI_OPTIONAL_WORDS
		ldy fp			; check floating point stack
		bmi Throw_FPOverflow
		cpy #FDim+1
		bcs Throw_FPUnderflow
  .endif

		rts			; all OK

 FHdr "PopA",NN ; """Pop TOS into A, with underflow check"""
PopA:		UnderflowCheck 1,Throw_Underflow
...
PopA2:		UnderflowCheck 2,Throw_Underflow
		lda DStack+0,x		; pop TOS to A (1 byte)
		inx
		inx
;	FEnd
...
PopYA:		UnderflowCheck 1,Throw_Underflow
...
PopTmp1: ; pop TOS to tmp1
		UnderflowCheck 1,Throw_Underflow
		lda DStack+0,x	; PopYA
		ldy DStack+1,x
		inx
		inx
...
Throw_Overflow:	lda #$100+err_Stack_Overflow
		bne ThrowA
...
_NotFound:	ldy #$ff		; print code
 ...
 FHdr "Empty-Stack",NN ; ( ? -- )  Empty data & FP stack
Empty_Stack:
		ldx #DStack0	; empty data stack
.if "fp" in TALI_OPTIONAL_WORDS
		lda #FDim	; empty FP stack
		sta fp
 .endif ; fp
	FEnd
		rts
...
_compile:	; We're compiling!
		...
		jsr Compile_Comma	; Compile the xt into the Dictionary
		jmp _loop
...
_not_immediate:	; This is not an immediate word
		...
		jsr Literal		; compile the xt of the word

		ldy #>Compile_Comma	; compile COMPILE,
		lda #<Compile_Comma
		jmp Jsr_Comma_YA
	FEnd
...
Compile_Comma_YA: jsr PushYA		; push xt
		...
		lda DStack+1,x		; Never Native (NN) word?
		and #NN
		bne _jsr_opt
...
_jsr_opt:
		...
		lda DStack+1,x		; underflow strip & not stack strip?
		eor #UF
		and #UF+R6
		bne _jsr

		lda #3			; skip the underflow check
		jsr Nos_plus_A
...
_inline: ; do native compile
		...
		lda DStack+1,x		; this word contains RTS addr save & restore?
		and #R6
		beq +
		...
		lda #$f4		;   Adjust u: omit the leading RTS addr save & trailing RTS addr restore
		jsr minus_a
+
		...
		lda #$fd		;   Adjust u: omit underflow check
		jsr minus_a
+
					; ( xt u )
...
Literal:
		...
		jmp Compile_Comma_YA	; compile JSR runtime
	FEnd
...
;		rts
 ...
 FHdr "LDYA#,",NN ; ( n -- )  compile "ldy #; lda #" 
ldya_immed_comma:
		lda #$a0		; ldy #
		ldy DStack+1,x
		jsr Comma_YA
...
lda_immed_comma: ; ( n -- ) compile "lda #"
 ...
 FHdr "PushYA",0 ; ( YA -- n )  Push YA to data stack
PushYA:		dex
		dex
		sta DStack+0,x
		sty DStack+1,x
...
False:		lda #0
 ...
 FHdr "PushZA",0 ; ( A -- n )  Push zero-extended A to the data stack
PushZA:		dex
		dex
		sta DStack+0,x
		lda #0
...
Bl:		lda #AscSP
 ...
 FHdr "dp",NN ; ( -- addr )  Variable: nt of current word
		lda #dp
		bne PushZA
	FEnd

 FHdr "Tmp1",NN ; ( -- addr ) Variable: zp work area
		lda #tmp1
		bne PushZA
	FEnd

 FHdr "DStack",NN ; ( -- addr ) Array variable: data stack
		lda #DStack
		bne PushZA
	FEnd


 FHdr "2Literal",UF+IM+NN ; (C: d -- ) ( -- d) "Compile a literal double word"
...
Exit:
		rts		; keep before FEnd so it gets inlined
	FEnd			; never reached
...
_colonword:
		...
		jsr Type		; print the offending word.
		jsr Space
...
; FHdr "'Nt",NN ; ( "name" -- nt )  Find a word's nt
...
Find:
		...
		; Save address in case conversion fails.
		lda DStack+1,x		; MSB
		pha
		lda DStack+0,x		; LSB
		pha
		...
		sta DStack+2,x
		pla
		sta DStack+3,x		; MSB of address

		rts			; ( addr 0 )

_found_word:
		pla			; We don't need the address after all, RDrop it
		pla
		...
		lda #Wh_Flags		; get flags
		jsr plus_a		; ( xt nt+Wh_Flags )
		lda (DStack+0,x)		; ( xt flags )
		and #IM
		bne _immediate		; bit set, we're immediate
...
find_name:
  ...
  ; FIND calls this word.
		...
		jsr fina_pattern_prepare ; tmp2 = pseudo-header of pattern name
...
_nextA:		jsr fina_search_wordlist
		beq _wordlist_next
...
fina_pattern_prepare:
		sec			; A= 0 - name start offset
		lda DStack+0,x
		sbc #wh_NameLastChar+1
		adc DStack+2,x		; tmp2 = pattern name nt
		sta tmp2+0
		lda #$ff
		adc DStack+3,x
		sta tmp2+1
...
_char_next:	iny			; to next char
		...
		and #$df		;   maybe only a case mismatch?
		bne _word_next
...
Two_variable:	jsr Variable		; compile word header & push PFA
...
Value:
 ...
 FHdr "2Value",UF+NN ; ( d "name" -- )  Define a 2Value word
...
_Err:		lda #$100+err_InvalidName ; unrecognized type.
 ...
 FHdr "DMax",NN ; ( d1 d2 -- d )  return highest, signed
  ; ANS double
...
DMax:		lda #$80
 ...
 FHdr "DMin",NN ; ( d1 d2 -- d )  return lowest, signed
  ; ANS double
...
+		eor tmp2
 ...
 FHdr "2Nip",UF+NN ; ( db da -- da ) "Delete NOS, double"
...
D_Plus:
 ...
 FHdr "D1+",UF ; ( d1 -- d2 )  increment a double
...
+
 ...
 FHdr "D1-",UF ; ( d1 -- d2 )  Decrement a double
...
_process_name:
		...
		lda dp+0		; compile wh_WordListLink
		ldy dp+1
		jsr Comma_YA
...
Key_A: ; returns char in A
 ...
 FHdr "Key?",NN ; ( -- f )  Is a key available?
...
Bounds:
		...
		clc
		lda DStack+0,x		; LSB u
		ldy DStack+2,x		; LSB addr
		adc DStack+2,x
		sta DStack+2,x		; LSB addr+u
		sty DStack+0,x

		lda DStack+1,x		; MSB u
		ldy DStack+3,x		; MSB addr
		adc DStack+3,x
...
Slash_String:	UnderflowCheck 3,Throw_Underflow_20
...
Throw_Underflow_20: jmp Throw_Underflow


 FHdr "2Drop",UF ; ( n n -- ) "Drop TOS and NOS"
; ## "2drop"  auto  ANS core
...
Two_drop:
 ...
 Fhdr "2Swap",UF+NN ; ( n1 n2 n3 n4 -- n3 n4 n1 n1 ) "Exchange two double words"
; ## "2swap"  auto  ANS core
	; """https://forth-standard.org/standard/core/TwoSWAP"""
Two_swap:	UnderflowCheck 4,Throw_Underflow_20
...
Two_over:	UnderflowCheck 4,Throw_Underflow_20
...
Two_Fetch_tmp1:
 ...
 FHdr "D@",NN ; ( addr -- d )  fetch double, XINU format
DFetch:		jsr PopYA
...
Abs:		UnderflowCheck 1,Throw_Underflow_17
...
DAbs:		UnderflowCheck 2,Throw_Underflow_17 ; double number
...
DEqual:		lda DStack+6,x
 ...
 FHdr "DU>",NN ; ( ud1 ud2 -- f )  return d1 > d2 (unsigned)
DUGt:		jsr DGtSub
		bcc True3
		bcs False3
	FEnd
...
DULe:		jsr DGtSub
 ...
 FHdr "DU>=",NN ; ( d1 d2 -- f )  Return d1 >= d2 (unsigned)
DUGe:		jsr DLessSub
		bcs True3
		bcc False3
	FEnd
...
Throw_Underflow_09: jmp Throw_Underflow
...
DGEq3:		bpl True3
  ...
  FHdr "D<",NN ; ( d1 d2 -- f )  return d1 < d2 (signed)
  ; https://forth-standard.org/standard/double/Dless
DLess:		jsr DLessSub
		bvs DGEq3
DLess3:		bpl False3
True3:		lda #$ff
DReturn3:	UnderflowCheck 4,Throw_Underflow_09
		sta DStack+6,x
		sta DStack+7,x
		jmp ThreeDrop
	FEnd
...
Throw_Underflow_08: jmp Throw_Underflow
...
Return1:	UnderflowCheck 2,Throw_Underflow_08
		inx		; Drop 
		inx
...
Throw_Underflow_11: jmp Throw_Underflow
...
Return0:	sta DStack+0,x
		sta DStack+1,x
		UnderflowCheck 1,Throw_Underflow_05
		rts
	FEnd
...
Throw_Underflow_05: jmp Throw_Underflow
...
Min:		UnderflowCheck 2,Throw_Underflow_05
...
Max:		UnderflowCheck 2,Throw_Underflow_05
...
+		rts
 ...
 FHdr "M+",NN ; ( d1 n -- d2 )  Return d1 + n
  ; https://forth-standard.org/standard/double/MPlus
MPlus:		jsr PopYA

		clc		; add n to lo word of d1
		adc DStack+2,x
		sta DStack+2,x
		tya
		adc DStack+3,x
		sta DStack+3,x
		bcc +		; propagate carry thru hi word of d1
		inc DStack+0,x
		bne +
		inc DStack+1,x
+
		tya		; if n negative
		bmi One_Minus_NoUf ;   decrement hi word of d1
	FEnd
		rts


 FHdr "D2*",UF ; ( d1 -- d2 )  Shift double left 1 bit
...
D2Star:		jsr underflow_2
 ...
 FHdr "D2/",UF ; ( d1 -- d2 )  Shift double right 1 bit, signed
...
D2SlashU:	ror DStack+1,x
 ...
 FHdr "UD2/",UF+NN ; ( ud1 -- ud2 )  Shift double right 1 bit, unsigned
...
UTwo_Slash:
 ...
 FHdr "DRShift",0 ; ( ud1 u -- ud2 )  shift double right u bits
...
+		jmp Type			; type formatted
 ...
 FHdr ".R",NN ; ( n u -- ) "Print NOS as unsigned number with TOS with"
...
Comma_YA:	; compile a word in YA (little-endian)
 ...
 FHdr "2,",NN ; ( d -- )  Append 2 cells in memory
Two_Comma:	jsr Comma
		jmp Comma
	FEnd


; FHdr "Drop,",NN ; ( -- )  Compile inline Drop
Drop_Comma:	lda #$e8	;inx
		tay
		bne Comma_YA
;	FEnd

 FHdr "Jsr,",NN ; ( addr -- )  compile JSR abs
...
Jsr_Comma_3:	jsr C_Comma_A
 ...
 FHdr "Jmp,",NN ; ( addr -- )  compile JMP abs
...
C_Store:	jsr underflow_2
 ...
 FHdr "1+!",NN ; ( addr -- )  increment cell at addr
OnePlusStore:
		lda (DStack+0,x)
		clc
		adc #1
...
_7:
 ...
 FHdr "+!",UF ; ( n addr -- ) "Add number to value at given address"
  ; ## "+!"  auto	 ANS core
  ; https://forth-standard.org/standard/core/PlusStore
Plus_store:
		jsr underflow_2

		jsr PopTmp1	; pop addr to tmp1
...
Execute:	UnderflowCheck 1,Throw_Underflow_03
...
TwoRot:		UnderflowCheck 6,Throw_Underflow_03
...
Throw_Underflow_03: jmp Throw_Underflow
 ...
 FHdr "Rot",NN ; ( a b c -- b c a ) "Rotate first three stack entries downwards"
  ...
  ; on top!
Rot:		UnderflowCheck 3,Throw_Underflow_03
...
Not_Rot:	UnderflowCheck 3,Throw_Underflow_03
...
Store:		UnderflowCheck 2,Throw_Underflow_03
...
Fetch:		UnderflowCheck 1,Throw_Underflow_03
...
; Assembler pseudo-instructions, directives and macros
 ...
 FHdr "push-a",IM+NN ; ( A -- n )  Push unsigned A onto data stack
 ...
 FHdr "-->",IM+NN ; ( -- addr )	 Save address
		jmp Here
	FEnd


 FHdr "<j",IM ; ( addr -- addr )  Calc absolute address
 ...
 FHdr "<b",IM+NN ; ( addr -- n )  Calc displacement
...
_WNext:		ldy #wh_WordListLink+1	;   next entry
		lda (tmp1),y
		beq _unknown
		pha
		dey
...
_WTest:		ldy #wh_Xt+3		;   opcode match?
		lda (tmp1),y
		cmp tmp2
		bne _WNext
				; found matching assembler opcode word

		lda tmp2		; save opcode
		pha
		...
		ldy #wh_xt+1		; get processor routine addr lo byte from JSR
		lda (tmp1),y
...
+		jsr PushYA
		...
		jsr Plus
		jsr Dup			; save for TypeSymbolN
					; ( addr_end addr dest dest )
		jsr _tab		; to source area
;		lda #'$'
;		jsr Emit_A
		jsr U_Dot		; print dest address
		pla			; get opcode word nt
		tay
		pla
		jsr _print_opcode
		pla			; discard opcode
		jsr TypeSymbolN		; print dest addr symbolic
		jmp _instr
...
;		jsr Emit_A
		...
		jsr Dup
		jsr TypeSymbolN		; print operand as symbol
